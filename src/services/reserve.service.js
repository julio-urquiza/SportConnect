    ...
    class ReserveService{
        ...
        consultarDisponibilidadCancha = async(id,fecha) =>{
            if(!id || !fecha) throw new CustomError(400,"No se recibio la información")
            const cancha= await courtDao.getById(id)
            const horarios= cancha.horariosDisponibles.filter(h => h.dia === createDay(fecha))
            const reservasOcupadas= await this.dao.getHorariosOcupados(id, fecha)
            const libres = horarios.filter(
                horario => !reservasOcupadas.some(cancha => cancha.horaInicio === horario.inicio)
            )
            if(libres.length === 0) throw new CustomError(404,"No se encontraron horarios disponibles para esa cancha")
            return libres
        }
